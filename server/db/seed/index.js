...
const frogImages = require('./rawFrogImages');
const { db, User, Product, Order } = require('../index');
  ...
  try {
    ...
    const products = await Promise.all(
      productData.map((product) =>
        Product.create({
          ...product,
          imageURL: frogImages[Math.floor(Math.random() * frogImages.length)],
        }),
      ),
    );
